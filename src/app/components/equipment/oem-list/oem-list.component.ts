...
@Component({
  ...
  styleUrl: './oem-list.component.css'
})
export class OemListComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private OEMService: OEMService, private toastService: ToastService) {
  ...
  fetchData() {
    ...
    this.OEMService.getAllOEMs(this.payload).subscribe({
      next: (response) => {
  ...
  deleteFn(id: number): void {
    if (id <= 0) return;
    const confirmed = window.confirm('Are you sure you want to delete this OEM?');
    if (confirmed) {
      this.OEMService.deleteOEM(id).subscribe({
        next: (response) => {
