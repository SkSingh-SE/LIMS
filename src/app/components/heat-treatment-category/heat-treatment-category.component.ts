...
export class HeatTreatmentCategoryComponent implements OnInit {
  ...
  heatTreatmentCategoryId: number = 0;
  formTitle = 'Heat Treatment Category Form';

  constructor(private fb: FormBuilder, private router: Router, private route: ActivatedRoute, private heatTreatmentCategoryService: HeatTreatmentCategoryService, private toastService: ToastService) {
  ...
  fetchData() {
    this.heatTreatmentCategoryService.getAllHeatTreatmentCategories(this.payload).subscribe({
      next: (response) => {
        this.HeatTreatmentCategoryList = response?.items || [];
      ...
      error: (error) => {
        ...
        this.HeatTreatmentCategoryList = [];
}
    }
  ...
  deleteFn(id: number): void {
    ...
    if (confirmed) {
      this.heatTreatmentCategoryService.deleteHeatTreatmentCategory(id).subscribe({
        next: (response) => {
          this.fetchData();
  ...
  openModal(type: string, id: number): void {
    ...
    if (type === 'create') {
      ...
      this.isEditMode = false;
      this.formTitle = 'View Heat Treatment Category';
      this.HeatTreatmentCategoryForm.disable();
    }
