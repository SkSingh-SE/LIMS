...
export class CustomMaterialSpecificationListComponent implements OnInit {
 ...
 columns = [
  ...
  fetchData() {
    ...
    this.materialSpecificationService.getAllCustomMaterialSpecifications(this.payload).subscribe({
      next: (response) => {
  ...
  deleteMaterialSpecification(id: number): void {
    ...
    if (confirmed) {
      this.materialSpecificationService.deleteMaterialSpecification(id).subscribe({
        ...
        error: (error) => {
          this.toastService.show(error.error?.message || error.message || 'Delete failed', 'error');
        }
      });
    }
  }
