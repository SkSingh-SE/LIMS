...
@Component({
    selector: 'app-document-change-request-preview',
    standalone: true,
    imports: [CommonModule, RouterModule, NablPrintHeaderComponent, NablPrintFooterComponent, PrintFrameComponent],
    templateUrl: './document-change-request-preview.component.html',
    styleUrl: './document-change-request-preview.component.css'
})
export class DocumentChangeRequestPreviewComponent implements OnInit {
    recordId: number = 0;
    data: any = null;
    ...
    printPage(): void {
        document.getElementById('dcr-print-size')?.remove();
        const styleEl = document.createElement('style');
        styleEl.id = 'dcr-print-size';
    ...
    goBack() { this.router.navigate(['/document-change-request']); }
}
