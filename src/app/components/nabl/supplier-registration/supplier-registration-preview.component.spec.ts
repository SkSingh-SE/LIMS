// @ts-nocheck
import { ComponentFixture, TestBed } from '@angular/core/testing';
import { RouterTestingModule } from '@angular/router/testing';
import { HttpClientTestingModule } from '@angular/common/http/testing';
import { ActivatedRoute, Router, convertToParamMap } from '@angular/router';
import { of, throwError } from 'rxjs';
import { CUSTOM_ELEMENTS_SCHEMA, ChangeDetectorRef } from '@angular/core';

import { SupplierRegistrationPreviewComponent } from './supplier-registration-preview.component';
import { SupplierRegistrationService } from '../../../services/supplier-registration.service';

describe('SupplierRegistrationPreviewComponent', () => {
  let component: SupplierRegistrationPreviewComponent;
  let fixture: ComponentFixture<SupplierRegistrationPreviewComponent>;
  let serviceSpy: jasmine.SpyObj<SupplierRegistrationService>;
  let router: Router;

  const mockData = {
    id: 1,
    supplierName: 'ABC Supplies',
    contactPerson: 'John',
    formatNo: 'F-19',
  };

  beforeEach(async () => {
    serviceSpy = jasmine.createSpyObj('SupplierRegistrationService', ['getById']);
    serviceSpy.getById.and.returnValue(of(mockData as any));

    await TestBed.configureTestingModule({
      imports: [SupplierRegistrationPreviewComponent, RouterTestingModule, HttpClientTestingModule],
      providers: [
        { provide: SupplierRegistrationService, useValue: serviceSpy },
        {
          provide: ActivatedRoute,
          useValue: {
            snapshot: { params: { id: '1' }, paramMap: convertToParamMap({ id: '1' }) },
            paramMap: of(convertToParamMap({ id: '1' })),
          },
        },
        { provide: ChangeDetectorRef, useValue: { detectChanges: jasmine.createSpy('detectChanges') } },
      ],
      schemas: [CUSTOM_ELEMENTS_SCHEMA],
    }).compileComponents();

    router = TestBed.inject(Router);
    fixture = TestBed.createComponent(SupplierRegistrationPreviewComponent);
    component = fixture.componentInstance;
  });

  it('should create', () => {
    fixture.detectChanges();
    expect(component).toBeTruthy();
  });

  it('should load data when id is provided', () => {
    fixture.detectChanges();
    expect(serviceSpy.getById).toHaveBeenCalledWith(1);
    expect(component.record()).toEqual(mockData as any);
  });

  it('should navigate away when data not found (null response)', () => {
    serviceSpy.getById.and.returnValue(of(null as any));
    fixture.detectChanges();
    // Component sets signal to null without navigating
    expect(component.record()).toBeNull();
  });

  it('should navigate away on service error', () => {
    serviceSpy.getById.and.returnValue(throwError(() => new Error('Server error')));
    fixture.detectChanges();
    // Component sets isLoading to false on error
    expect(component.isLoading()).toBeFalse();
  });

  it('should default to portrait orientation', () => {
    fixture.detectChanges();
    expect(component.orientation).toBe('portrait');
  });

  it('should set orientation manually', () => {
    fixture.detectChanges();
    component.setOrientation('landscape');
    expect(component.orientation).toBe('landscape');
    expect(component.orientationManual).toBeTrue();
  });

  it('should reset to auto orientation', () => {
    fixture.detectChanges();
    component.setOrientation('landscape');
    component.resetToAuto();
    expect(component.orientationManual).toBeFalse();
  });

  it('goBack should navigate to list', () => {
    fixture.detectChanges();
    const navigateSpy = spyOn(router, 'navigate');
    component.goBack();
    expect(navigateSpy).toHaveBeenCalledWith(['/supplier-registration']);
  });
});
