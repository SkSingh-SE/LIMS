...
export class ProductFormComponent implements CanComponentDeactivate, OnInit {
  ...
  fetchData() {
    this.service.getAllProductForms(this.payload).subscribe({
      next: (response) => {
        this.ProductFormList = response?.items || [];
  ...
  deleteFn(id: number): void {
    ...
    if (confirmed) {
      this.service.deleteProductForm(id).subscribe({
        next: (response) => {
          this.fetchData();
  ...
  openModal(type: string, id: number): void {
    ...
    if (type === 'create') {
      ...
      this.isEditMode = false;
      this.formTitle = 'View Product Form';
      this.ProductFormForm.disable();
    }
